...
class GetAccel {
	public:
		GetAccel GetAccel(int data, int magS, const int addr, const int id);
	...
	private:
		...
		const int MMA8653_ADDRESS = (0x0d<<1); // bit shifted to fit 7-bit buffer
};
...
GetAccel::GetAccel(int data, int magS, const int addr, const int id){
    ...
    data = dataBuffer;
	magS = magStatus;
	addr = MMA8653_ADDRESS;
	id = MMA8653_ID;
	data[0] = 0x0e;
	magS = i2c.write(MMA8653_ADDRESS,data,1,true);  // Write register number
	magS = i2c.read(MMA8653_ADDRESS,data,1); // Read register contents
	...
	data[0]=0x2a;
	data[1]=1;
	magS = i2c.write(MMA8653_ADDRESS,data,2);  // Write data to register
}

void GetAccel::getX (int data, const int8_t xC, int magS, int x) {
	data = dataBuffer;
	...
	while(1){
		...
		magS = i2c.write(MMA8653_ADDRESS,data,1,true);  // Write register number
		magS = i2c.read(MMA8653_ADDRESS,data,2); // Read register contents
		xCoord = data[0];
...
void GetAccel::getY (int data, const int8_t yC, int magS, int y) {
	data = dataBuffer;
...
void GetAccel::getZ (int data, const int8_t zC, int magS, int z) {
	data = dataBuffer;
