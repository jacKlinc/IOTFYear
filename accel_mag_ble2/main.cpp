...
static const uint16_t uuid16_list[] = {LEDService::LED_SERVICE_UUID, ACCELService::ACCEL_SERVICE_UUID, MagService::MAG_SERVICE_UUID};
...
void updateMag()
{
    char magData[8]; // Declare a buffer for data transfer    
    int Status;
        int16_t X;
        magData[0]=0x01; // Register number 1 has the X data (2 bytes)
        Status = i2c.write(MAG3110_ADDRESS,magData,1,true);  // Write register number
        Status = i2c.read(MAG3110_ADDRESS,magData,2); // Read register contents
        X = magData[0];
        X = (X << 8) + magData[1];
        X = X >> 6; // only 10 bits of data are available
        
        int16_t Y;
        magData[0]=0x03; // Register number 3 has the Y data (2 bytes)
        Status = i2c.write(MAG3110_ADDRESS,magData,1,true);  // Write register number
        Status = i2c.read(MAG3110_ADDRESS,magData,2); // Read register contents
        Y = magData[0];
        Y = (Y << 8) + magData[1];
        ...
        int16_t Z;
        magData[0]=0x05; // Register number 1 has the Z data (2 bytes)
        Status = i2c.write(MAG3110_ADDRESS,magData,1,true);  // Write register number
        Status = i2c.read(MAG3110_ADDRESS,magData,2); // Read register contents
        Z = magData[0];
        Z = (Z << 8) + magData[1];
...
int main(void)
{    
    char Data[8]; // Declare a buffer for accel    
    char magData[8]; // mag version
    int Status, magStatus;
    ...
    // Wake the mag from sleep mode by writing 1 to register number     
    magData[0]=0x10; 
    magData[1]=1;
    magStatus = i2c.write(MAG3110_ADDRESS,magData,2);  // Write data to register  
